...
## 6. Git / Commit / Experiment Version Rules

### 核心原则

本项目不强制每个调参实验都开分支，允许在 `main` 上快速开发和调参；但正式实验必须可追溯到明确的 commit SHA。文档中的 commit SHA 只作为历史记录。最终 GPT Pro review 版本以用户/Codex 最终回报的 `branch + head_sha` 为准。不要为了让文档记录自身最终 head SHA 而反复 docs-only commit，因为 commit SHA 只能在提交完成后获得。如果 `MODEL_DEV_CURRENT.md`、`Prompt.md` 或 `MODEL_DEV_HANDOFF.md` 中记录的 SHA 与用户最终给出的 `branch + head_sha` 不一致，以用户最终给出的 `branch + head_sha` 为准。
